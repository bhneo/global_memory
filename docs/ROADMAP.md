...
- 验收：111 tests、doctor/lint/raw integrity、migration dry-run、M6 nine-scenario demo。（以本轮最终实测为准）
- M6.1：follow-up locator 规范化、去重、dry-run/备份/幂等迁移与 superseded lineage。（完成；活动任务 11 → 10）
- 后续：人工拆分 3 个 compound claims；核验 23 个 partial-entailment claims；关闭 10 个 primary follow-ups；分批批准高价值 concept/question/tension/synthesis。
