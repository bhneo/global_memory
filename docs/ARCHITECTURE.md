...
M8.1.1 adds an explicit qualification dimension inside Trusted: current-policy qualified versus awaiting requalification. A policy-version change never bulk-demotes Trusted. Governed Trusted writes use `prepared -> staged -> receipt_completed -> event_logged -> finalized`; recovery rolls back before Receipt completion and roll-forwards the audit boundary after it. Canonical evolution has no direct-write path and is always represented by a reviewable update Proposal plus an Exception when contradictory evidence exists.
