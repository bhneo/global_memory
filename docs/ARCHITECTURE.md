...
`gm context` 位于读取边界，不是 processor 或写入路径。它仅依据明确 query 与 token budget 对 `source`、`claim`、`synthesis` 的检索命中做确定性裁剪；输出保留路径、文档 hash、来源 ID 和选择理由。Context Pack 不写入 Markdown、SQLite 或审计日志，不缓存为真相层，也不提升任何命中或摘录的认知状态。
